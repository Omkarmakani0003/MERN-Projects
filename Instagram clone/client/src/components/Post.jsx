     ...
     try{
        ...
        if(response.data.success){
          if(response.data.data){
            document.querySelector(`.likeBtn_${post._id}`).classList.add('liked')
            document.querySelector(`.likeCount_${post._id}`).innerText = parseInt(document.querySelector(`.likeCount_${post._id}`).innerText) + 1
          }else{
            document.querySelector(`.likeBtn_${post._id}`).classList.remove('liked')
            document.querySelector(`.likeCount_${post._id}`).innerText = parseInt(document.querySelector(`.likeCount_${post._id}`).innerText) - 1
          }
        }
        ...
        <article className="post-card">
                ...
                <div className="post-actions">
                  <div className="post-actions-left">
                    <button className={`action-btn like-btn likeBtn_${post._id} ${isLiked ? 'liked' : '' }`} onClick={likeDislikeHandler}>
                      ...
                      <span className={`like-count likeCount_${post._id}`}>{post.likes_count}</span>
                    </button>
